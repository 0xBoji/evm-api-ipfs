...
const imgPath = computed(() => {
  return isGnosisSafe.value ? '/logo/logo-new.png' : '/logo/logo.png';
});
</script>
        ...
        <div class="ml-3 flex flex-auto items-center">
          ...
          >
            <div class="flex h-full flex-grow items-center">
              <img
                :src="imgPath"
                class="rounded-full bg-skin-border object-cover h-[20px] sm:h-[30px] md:h-[40px]"
                alt="Trustcore"
                style="background-color: #ffffff"
              />
              <span class="px-[22px]">Trustcore</span>
            </div>
          </router-link>
        </div>
